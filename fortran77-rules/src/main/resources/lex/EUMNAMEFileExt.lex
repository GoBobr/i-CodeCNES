...
/********************************************************************************/
/* This file is used to generate a rule checker for EUM.NAME.FileExt rule.      */
/* File extension must be .f90, .F90, .f95, .F95, .f03, or .F03.                */
/********************************************************************************/
...
%{
    private static final Logger LOGGER = Logger.getLogger(EUMNAMEFileExt.class.getName());

	String location = "MAIN PROGRAM";
    String parsedFileName;

	public EUMNAMEFileExt(){
	}
	...
	public void setInputFile(final File file) throws FileNotFoundException {
        ...
        Pattern extPattern = Pattern.compile("\\.(f|F|for|FOR|f77|F77|f90|F90|f95|F95|f03|F03)$");
        Matcher matcher = extPattern.matcher(file.getName());
        if(!matcher.find()) {
            LOGGER.fine("Setting error: invalid file extension");
            setError(location, "File extension must be .f, .F, .for, .FOR, .f77, .F77, .f90, .F90, .f95, .F95, .f03, or .F03.", 1);
        }
