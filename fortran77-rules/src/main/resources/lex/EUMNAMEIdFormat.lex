...
/********************************************************************************/
/* This file is used to generate a rule checker for EUM.NAME.IdFormat.          */
/* Identifiers must follow [scope_][type]<name> format where type is a Hungarian*/
/* prefix (c=character, i=integer, r=real, x=double, l=logical, t=type,         */
/* a=allocatable, p=pointer). Name is CamelCase or ALL_CAPS for constants.      */
/********************************************************************************/
...
DOUBLE_COLON = "::"
HUNGARIAN_PREFIX = [cirxltapCIRXLTAP]

%{
    private static final Logger LOGGER = Logger.getLogger(EUMNAMEIdFormat.class.getName());

    String location = "MAIN PROGRAM";
    String parsedFileName;
    int declLine = 0;

    public EUMNAMEIdFormat() {}
    ...
    private void checkIdentifier(String varname) {
        if(varname.length() >= 2 && !varname.equals(varname.toUpperCase())) {
            ...
            if("cirxltapCIRXLTAP".indexOf(first) < 0) {
                setError(location, "Identifier '" + varname + "' must follow [scope_][type]<name> format with Hungarian type prefix (c,i,r,x,l,t,a,p).", declLine);
            }
        }
    }
%}
