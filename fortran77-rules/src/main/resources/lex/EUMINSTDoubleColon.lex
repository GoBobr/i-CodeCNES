...
/********************************************************************************/
/* This file is used to generate a rule checker for EUM.INST.DoubleColon.       */
/* Double colon (::) must be present in all type declarations.                  */
/********************************************************************************/
...
%{
    ...
    private static final Logger LOGGER = Logger.getLogger(EUMINSTDoubleColon.class.getName());
    String parsedFileName;
    int declLine = 0;

    public EUMINSTDoubleColon() {}
...
/* DECL STATE           */
/************************/
/* After matching a declaration keyword, look for :: on the same line */
/* Note: use \! for inline comments, not {COMMENT_WORD}, because */
/* {COMMENT_WORD} includes c/C which conflicts with variable names */
<DECL>          {DOUBLE_COLON}  {yybegin(LINE);}
<DECL>          \n              {setError(location, "Declaration is missing double colon (::).", declLine); yybegin(NEW_LINE);}
<DECL>          \!              {setError(location, "Declaration is missing double colon (::).", declLine); yybegin(COMMENT);}
<DECL>          .               {}
