...
%{
    private static final Logger LOGGER = Logger.getLogger(EUMINSTPercentBlank.class.getName());
...
<NEW_LINE>		{SPACE_PERCENT}	{
                                    LOGGER.fine("Setting error line "+(yyline+1)+" due to space before %");
                                    setError(location,"No spaces are allowed around the % character.", yyline+1);
                                }
<NEW_LINE>		{PERCENT_SPACE}	{
                                    LOGGER.fine("Setting error line "+(yyline+1)+" due to space after %");
                                    setError(location,"No spaces are allowed around the % character.", yyline+1);
                                }
<NEW_LINE>  	\n             	{}
