...
/********************************************************************************/
/* This file is used to generate a rule checker for EUM.INST.DummyArgOrder.     */
/* Declaration order of dummy arguments must match the argument list order in   */
/* the subroutine/function definition.                                          */
/********************************************************************************/
...
%{
    private static final Logger LOGGER = Logger.getLogger(EUMINSTDummyArgOrder.class.getName());
    ...
    private void checkArgOrder() {
        List<String> declOrder = new ArrayList<String>();
        for(String var : declArgs) {
            if(funcArgs.contains(var)) {
                declOrder.add(var);
            }
        }
        for(int i = 0; i < declOrder.size() && i < funcArgs.size(); i++) {
            if(!declOrder.get(i).equals(funcArgs.get(i))) {
                setError(location, "Declaration order of dummy arguments must match argument list order.", errorLine);
                break;
            }
        }
        funcArgs.clear();
...
<YYINITIAL>     {END_FUNC}      {checkArgOrder(); yybegin(LINE);}
<YYINITIAL>     \n              {yybegin(NEW_LINE);}
...
<NEW_LINE>      {END_FUNC}      {checkArgOrder(); yybegin(LINE);}
<NEW_LINE>      \n              {yybegin(NEW_LINE);}
