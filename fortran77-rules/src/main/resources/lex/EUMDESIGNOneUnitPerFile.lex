...
/********************************************************************************/
/* This file is used to generate a rule checker for EUM.DESIGN.OneUnitPerFile.  */
/* Each file must contain only one programming unit (PROGRAM or MODULE).        */
/********************************************************************************/
...
STRING		 = \'[^\']*\' | \"[^\"]*\"
UNIT		 = {PROG} | {MOD}
END_UNIT	 = end [ \t]* ({PROG}|{MOD})
%{
    private static final Logger LOGGER = Logger.getLogger(EUMDESIGNOneUnitPerFile.class.getName());

	String location = "MAIN PROGRAM";
    String parsedFileName;
    int unitCount = 0;

	public EUMDESIGNOneUnitPerFile(){
	}
...
%eofval{
  	if(unitCount > 1) {
  		LOGGER.fine("Setting error: multiple programming units in file");
  		setError(location, "Each file must contain only one programming unit (PROGRAM or MODULE).", 1);
  	}
...
<YYINITIAL>  	{COMMENT_WORD} 	{
                                    ...
                                    LOGGER.fine("["+this.parsedFileName+":"+(yyline+1)+":"+yycolumn+"] - YYINITIAL -> LINE (Transition : END_UNIT \""+yytext()+"\" )");
                                    yybegin(LINE);
                                }<YYINITIAL>		{UNIT}        	{
                                    location = yytext();
                                    unitCount++;
                                    LOGGER.fine("["+this.parsedFileName+":"+(yyline+1)+":"+yycolumn+"] - YYINITIAL -> NAMING (Transition : UNIT \""+yytext()+"\" )");
                                    yybegin(NAMING);
                                }
<YYINITIAL>		{TYPE}        	{
                                    location = yytext();
...
<NEW_LINE>		{STRING}		{}<NEW_LINE>		{END_UNIT}      	{
                                    LOGGER.fine("["+this.parsedFileName+":"+(yyline+1)+":"+yycolumn+"] - NEW_LINE -> LINE (Transition : END_UNIT \""+yytext()+"\" )");
                                    yybegin(LINE);
                                }<NEW_LINE>		{UNIT}        	{
                                    location = yytext();
                                    unitCount++;
                                    LOGGER.fine("["+this.parsedFileName+":"+(yyline+1)+":"+yycolumn+"] - NEW_LINE -> NAMING (Transition : UNIT \""+yytext()+"\" )");
                                    yybegin(NAMING);
                                }
<NEW_LINE>		{TYPE}        	{
                                    location = yytext();
...
/* LINE STATE    	    */
/************************/
<LINE>			{STRING}		{}<LINE>			{END_UNIT}      	{}<LINE>			{UNIT}        	{
                                    location = yytext();
                                    unitCount++;
                                    LOGGER.fine("["+this.parsedFileName+":"+(yyline+1)+":"+yycolumn+"] - LINE -> NAMING (Transition : UNIT \""+yytext()+"\" )");
                                    yybegin(NAMING);
                                }
<LINE>			{TYPE}        	{
                                    location = yytext();
