...
%{
    private static final Logger LOGGER = Logger.getLogger(EUMPRESBlankLines.class.getName());
...
<YYINITIAL> 	\n             	{
                                    LOGGER.fine("["+this.parsedFileName+":"+(yyline+1)+":"+yycolumn+"] - YYINITIAL -> BLANK (Transition : \\n )");
                                    blankCount = 1;
                                    yybegin(BLANK);
                                }
<YYINITIAL> 	.              	{
...
<NEW_LINE>  	\n             	{
                                    LOGGER.fine("["+this.parsedFileName+":"+(yyline+1)+":"+yycolumn+"] - NEW_LINE -> BLANK (Transition : \\n )");
                                    blankCount = 1;
                                    yybegin(BLANK);
                                }
...
<BLANK>			\n				{
								...
								if(blankCount > 2 && !errorReported) {
									LOGGER.fine("Setting error line "+(yyline+1)+" due to more than 2 consecutive blank lines");
