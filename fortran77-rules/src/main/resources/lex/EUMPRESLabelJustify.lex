...
INDENTED_LABEL = [ \t]+ [0-9]+
%{
    private static final Logger LOGGER = Logger.getLogger(EUMPRESLabelJustify.class.getName());
...
<NEW_LINE>		{INDENTED_LABEL}	{
							LOGGER.fine("Setting error line "+(yyline+1)+" due to label not at column 1");
							setError(location,"Labels must start at column 1.", yyline+1);
							yybegin(LINE);
						}
<NEW_LINE>		{LABEL}			{ yybegin(LINE); }
<NEW_LINE>  	\n             	{}
