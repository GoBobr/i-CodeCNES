...
%{
    private static final Logger LOGGER = Logger.getLogger(EUMINSTContinuation.class.getName());
...
<NEW_LINE>		{BAD_AMP}		{
                                    /* In F77, & at column 6 (0-indexed: 5) is a valid continuation char */
                                    if(yycolumn >= 6) {
                                        LOGGER.fine("Setting error line "+(yyline+1)+" due to mid-line ampersand");
                                        setError(location,"The ampersand & must be placed only at the end of continued lines.", yyline+1);
                                    }
                                }
<NEW_LINE>		{AMP_COMMENT}	{
                                    if(yycolumn >= 6) {
                                        LOGGER.fine("Setting error line "+(yyline+1)+" due to ampersand followed by comment");
                                        setError(location,"The ampersand & must be placed only at the end of continued lines.", yyline+1);
                                    }
                                }
<NEW_LINE>  	\n             	{}
