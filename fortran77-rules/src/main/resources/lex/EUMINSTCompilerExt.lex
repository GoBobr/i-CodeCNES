...
%{
    private static final Logger LOGGER = Logger.getLogger(EUMINSTCompilerExt.class.getName());
...
<NEW_LINE>		{DEC_PRAGMA}	{
                                    LOGGER.fine("Setting error line "+(yyline+1)+" due to compiler extension pragma");
                                    setError(location,"Compiler extensions are not allowed.", yyline+1);
                                    yybegin(COMMENT);
                                }
<NEW_LINE>		{BYTE_KW}		{
                                    LOGGER.fine("Setting error line "+(yyline+1)+" due to BYTE keyword");
                                    setError(location,"Compiler extensions are not allowed.", yyline+1);
                                }
<NEW_LINE>		{VAL_REF}		{
                                    LOGGER.fine("Setting error line "+(yyline+1)+" due to %VAL/%REF/%LOC");
                                    setError(location,"Compiler extensions are not allowed.", yyline+1);
                                }
<NEW_LINE>  	\n             	{}
